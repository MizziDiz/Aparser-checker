...
- **Проверка пересылки** — тест именно через релей (клиент → релей → Telegram); нужен заполненный `telegram_relay_url` и `relay_secret`. Для клиентов.
- **Запуск реле / Остановка реле** — поднять/остановить сервер-релей прямо из окна. **Только на сервере-релее, не на клиентах.** Релей работает, пока открыто окно редактора; для постоянной работы используйте задачу планировщика с `--relay` (см. раздел про релей).
