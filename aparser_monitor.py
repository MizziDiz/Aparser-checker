...
DEFAULTS = {
    ...
    "relay_bind": "0.0.0.0",  # на сервере-релее: адрес прослушивания (лучше — IP локальной сети)
    "relay_allowed_ips": "",  # на релее: разрешённые IP/подсети, напр. "10.10.10.0/24"; пусто — все
    ...
    "completion_confirm_reads": 2,  # подтвердить «активных 0» N проходами до «завершено»
                                    # (защита от ложного нуля при флейковом чтении UI)
    "request_timeout": 30,
    ...
    "stats_settle_min": 2,        # статистику по результату считаем, когда он не менялся N мин
    "stats_snapshots": True,      # писать снимки метрик заданий в БД в проходе монитора
    "stats_retention_days": 30,   # удалять данные статистики старше N дней (0 — не удалять)
    ...
    "top_period_days": 7,         # период топ-сводки в Telegram (--top)
    "top_limit": 10,              # сколько позиций показывать в каждом топе
