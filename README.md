...
Ключи: `autopilot_create_tasks` (вкл; по умолчанию **выкл** → поведение Phase 1 с уведомлением), `autopilot_template_task`, `autopilot_start_task`, `autopilot_max_new_tasks`.

> **Статус:** оркестрация, выбор батчей, идемпотентность и проверка готовы; сам клик по форме создания в UI (`create_task_from_batch`) ждёт точных ExtJS-селекторов этой сборки A-Parser — снимите дампы экрана через `--interactive` (Copy task + Task Editor) и заполните шаги. До этого авто-создание не выполняется, автопилот ведёт себя как Phase 1.
