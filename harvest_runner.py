...
def unit_stats(source: str) -> dict[tuple[str, str], tuple[int, int]]:
    """{(seed,op): (new_на_последнем_прогоне, число_прогонов)} из БД по источнику."""
    if not DB.exists():
        return {}
    c = db_conn()
    tmp: dict[tuple[str, str], list] = {}
    for seed, op, ts, ns in c.execute(
            "SELECT seed, op, ts, SUM(new) FROM results WHERE source=? "
            "GROUP BY seed, op, ts ORDER BY ts", (source,)):
